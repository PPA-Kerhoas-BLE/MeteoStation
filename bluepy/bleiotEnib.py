...
class MyDelegate(DefaultDelegate):
    ...
    def handleNotification(self, hnd, data):
        global data_temperature, data_humidity, data_state, temp_float
        if (hnd == e_temperature_handle):
            ...
            temp_float = float(int(concat_dat,16))/10                       
            print(" Temp dec:"+str(temp_float))
            data_temperature = temp_float    
...
def askFordatas():
    ...
    strTemperature.set(str(temp_float) + ' --- ' + time.ctime())
...
def onPushSwitch():
    ...
    if ledstate == 0:
        print("switch on")
        bleiot.lightswitch.set_switch_led_on()
        ledstate = 1
    else:
        print("switch off")
        bleiot.lightswitch.set_switch_led_off()
        ledstate = 0
...
if  __name__ == "__main__" :
    ...
    bleiot.environment.enable()
    bleiot.environment.set_humidity_notification(True)       
    bleiot.lightswitch.enable()
    bleiot.lightswitch.set_ledstate_notification(True)    
...
#=======================================================================
# $ sudo python3 bleiot.py  02:80:e1:00:00:ab --temperature -n 10000
