...
class MyDelegate(DefaultDelegate):
    ...
    def handleNotification(self, hnd, data):
        ...
        if (hnd == e_temperature_handle):
            ...
            #updateDisplay(str(data))   
           # strTemperature.set(str(data) + ' --- ' + time.ctime())     
        elif (hnd == e_humidity_handle):
            teptep = binascii.b2a_hex(data)
            print('Notification: Humidity received: {} %'.format(self._str_to_int(teptep)))      
            #strHumidity.set(str(data) + ' --- ' + time.ctime()) 
        elif (hnd == ledstate_handle):   
            strLedState.set(str(data) + ' --- ' + time.ctime())
...
#def updateDisplay(myString_temperature):
#    strTemperature.set(myString_temperature) 
# strTemperature.set(str(myString) + ' --- ' + time.ctime())
